...
class MoatBreakdown:
    ...
    def to_html(self) -> str:
        ...
        return (
            ...
            '<thead><tr>'
            '<th style="text-align:left;color:#7a8aa6;padding:6px 10px;font-size:0.72rem;">Component</th>'
            '<th style="text-align:right;color:#7a8aa6;padding:6px 10px;font-size:0.72rem;">Score</th>'
            '<th style="text-align:left;color:#7a8aa6;padding:6px 10px;font-size:0.72rem;">Why</th>'
            '</tr></thead><tbody>'
            + "".join(rows)
            + '</tbody></table>'
        )
...
def _component_roic(fin: pd.DataFrame, wacc_baseline: float = 0.09) -> dict:
    ...
    if ebit.empty or len(ebit.dropna()) < 2:
        return {"value": None, "points": 1.0, "why": "Insufficient data"}
    ...
    if excess >= 0.15:
        pts, why = 2.0, f"ROIC {avg*100:.1f}% — exceptional"
    elif excess >= 0.08:
...
def _component_gross_margin(fin: pd.DataFrame) -> dict:
    ...
    if mean_gm >= 0.55: lvl = 1.0
    elif mean_gm >= 0.40: lvl = 0.7
    elif mean_gm >= 0.25: lvl = 0.4
    else: lvl = 0.1
    stb = max(0.0, min(1.0, 1.0 - cv * 4))
    pts = lvl + stb
    why = f"Avg GM {mean_gm*100:.1f}% · CV {cv*100:.1f}%"
...
def _component_op_margin(fin: pd.DataFrame) -> dict:
    ...
    if avg >= 0.30: pts, why = 2.0, f"Avg OM {avg*100:.1f}% — exceptional"
    elif avg >= 0.20: pts, why = 1.6, f"Avg OM {avg*100:.1f}% — strong"
    elif avg >= 0.12: pts, why = 1.2, f"Avg OM {avg*100:.1f}% — solid"
    elif avg >= 0.06: pts, why = 0.6, f"Avg OM {avg*100:.1f}% — modest"
    else: pts, why = 0.0, f"Avg OM {avg*100:.1f}% — weak"
    return {"value": avg, "points": pts, "why": why}
...
def _component_growth(fin: pd.DataFrame) -> dict:
    ...
    if mean_g >= 0.10 and std_g <= 0.10: pts, why = 2.0, f"Mean growth {mean_g*100:.1f}% consistent"
    elif mean_g >= 0.05: pts, why = 1.4, f"Mean growth {mean_g*100:.1f}% — steady"
    elif mean_g >= 0.0: pts, why = 0.8, f"Mean growth {mean_g*100:.1f}% — slow"
    else: pts, why = 0.0, f"Mean growth {mean_g*100:.1f}% — declining"
...
def _component_cash_conversion(fin: pd.DataFrame) -> dict:
    ...
    if avg >= 1.20: pts, why = 2.0, f"CFO/NI {avg:.2f} — excellent cash quality"
    elif avg >= 1.00: pts, why = 1.5, f"CFO/NI {avg:.2f} — earnings turn into cash"
    elif avg >= 0.80: pts, why = 0.8, f"CFO/NI {avg:.2f} — modest"
    else: pts, why = 0.0, f"CFO/NI {avg:.2f} — accruals risk"
    return {"value": avg, "points": pts, "why": why}
...
def derive_moat(fin: pd.DataFrame, wacc_baseline: float = 0.09) -> MoatBreakdown:
    ...
    if score >= 8: verdict = "Wide moat — compounding machine"
    elif score >= 6: verdict = "Strong moat — durable advantages"
    elif score >= 4: verdict = "Modest moat — average competitive position"
    elif score >= 2: verdict = "Narrow moat — fragile economics"
    else: verdict = "No moat — commodity-like business"
    return MoatBreakdown(score=score, components=components, rationale=f"{score}/10 · {verdict}")
